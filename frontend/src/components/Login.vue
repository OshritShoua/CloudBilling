<template>
  <div>
    <div v-if="!loggedIn" class="card card-container">
      <p v-if="authFailed">Invalid User Name or Password</p>
      <img id="profile-img" class="profile-img-card" src="//ssl.gstatic.com/accounts/ui/avatar_2x.png" />
      <p id="profile-name" class="profile-name-card"></p>
      <form class="form-signin">{{user.username}} {{user.password}}
        <input v-model="user.username" type="text" name="username" class="form-control" placeholder="User Name" required autofocus>
  ...
  export default {
    ...
    methods : {
      login () {
        ...
        var params = 'username='+this.user.username+'&password='+this.user.password;
